...
### Bootstrap logger
A fixed-severity logger used at process entry when configuration is not yet available, so fatal startup failures (config validation, listen errors) still emit on stderr before exit. Created with a hard-coded error level in the `loadConfig` catch path because the configured log level cannot be read when parsing fails.
